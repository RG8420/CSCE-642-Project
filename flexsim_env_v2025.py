...
class FlexSimEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None): # <-- FIX: Add 'options=None' # For continuous models
        # Alternative robust fix: def reset(self, seed=None, **kwargs):
        super().reset(seed=seed)
        self.seedNum = self.np_random.integers(1, 2147483647)
        self._reset_flexsim()
        state, reward, done = self._get_observation()
        info = {}
        # In Gymnasium, the reset method must return (observation, info)
        return state, info
...
def main():
    ...
    env = FlexSimEnv(
        flexsimPath = r"C:\Program Files\FlexSim 2024\program\flexsim.exe",
        modelPath = r"C:\RL_project\CSCE642_RL_Project\FlexSim_JSP_Model_v1.fsm",
        verbose = True,
        visible = True
        )
